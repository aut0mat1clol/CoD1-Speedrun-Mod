...
//		setcvar("debug_notank", "on");
	...
	//hack.. changes models to cammo models.
	array = [];
	...
	{
		array[array.size] = vehicles[i];
	}
	for(i=0;i<array.size;i++)
	{
		if(isdefined(array[i].model))
		{
			if(array[i].model == "xmodel/vehicle_tank_tiger")
				array[i] setmodel  ("xmodel/vehicle_tank_tiger_snow");
			else if(array[i].model == "xmodel/vehicle_tank_panzerIV" || array[i].model == "xmodel/vehicle_tank_panzeriv")
				array[i] setmodel  ("xmodel/vehicle_tank_panzerIV_camo");
		}
	}
	//////////end hack
	maps\_tank::main();
	...
	playertank maps\_t34::init_player();
	playertank thread maps\_tankgun::mginit();
	...
	level.playertank thread maps\_treads::main();
	level.playertank thread watersplashes();
	if(getcvar("debug_notank") == "off")
		playertank useby(level.player);
	//	playertank thread bouncy();
	...
	playertank thread tank_getmovingmessage();
	...
	for(i=0;i<tanks.size;i++)
	{
		if(isdefined(tanks[i].script_team))
		{
			...
			{
				tanks[i] maps\_tank::setteam("axis");
				tanks[i] maps\_panzerIV::init();
				tanks[i] thread enemy_tanks_think();
			}
			if(tanks[i].script_team == "friendly")
			{
				tanks[i] maps\_tank::setteam("allies");
				tanks[i] maps\_t34::init_friendly();
...
bouncy()
{
	while(1)
	{
		...
		vector = maps\_utility::vectorMultiply(vector,500);
		org = self.origin;
		aheadsite = org + vector;
		radiusDamage (aheadsite, 2000, 900, 800);
		//self doDamage ( 500, (0,43,22));
		wait 1;
	}
}
...
tank_hud()
{
	...
	tankhud2.x = 635;
	tankhud2.y = 450;
	tankhud2.sort = 1000;   // [SR] frame must draw above the fill: engine reorders equal-sort hudelems when extra script elems exist
...
//	thread maps\_tank::treads();
	...
	self thread maps\_tank::avoidtank();
...
attacktarget(trigger,target)
{
	...
	if(!isdefined(target))
		return;
	target thread maps\_tank::queadd(self);
//	target.guyshootingatme = self;  //fightback
	...
	if(target.health > 0)
		self thread maps\_tank::queadd(target);
...
//		self queallenemies();
	...
	target thread maps\_tank::queadd(self);  //fight back
	if(!isdefined(trigger))
	{
		speed = 0;
	}
...
setturretforward()
{
	...
	while(!isdefined(self.enemyque) && count < 8)
	{
		count++;
		vector = anglesToForward(self.angles);
		vector = maps\_utility::vectorMultiply(vector,8000);
...
trigger_spawners_wait(ent,tank,delay)
{
	...
	if(isdefined(delay))
		wait delay;
	vehicle = maps\_vehiclespawn::vehicle_spawn(ent.target);
...
//		}
	...
	else if(vehicle.vehicletype == "tiger")
		vehicle thread enemy_tanks_think();
	else
	{
		spam();
...
//	self endon ("death");
	...
	{
		self waittill ("trigger",other);
		if(other.targetname != "player_tank" && isdefined(other.script_team) && other.script_team != "axis")
		{
			break;
		}
	}
	...
	if(isdefined(targ))
	{
		other thread maps\_tank::queadd(targ);
	}
	if(other.health > 0)
...
focusfire(target)
{
	...
	self endon ("deadstop");
	self setspeed (5,15);
	maps\_tank::queaddtofront(target);
	target waittill ("death");
...
reenforcements_ondeath(tank)
{
	...
	vehicle = maps\_vehiclespawn::vehicle_spawn(reenforcement);
...
queallenemies()
{
	...
	for(i=0;i<tanks.size;i++)
	{
		...
		if(isdefined(tanks[i].script_team) && tanks[i].script_team != self.script_team)
			self maps\_tank::queadd(tanks[i]);
	}
}
...
dialog()
{
	enemyspottedtrigs = getentarray("enemyspotter","targetname");
	for(i=0;i<enemyspottedtrigs.size;i++)
	{
		enemyspottedtrigs[i] thread trigger_enemy_spotter();
	}
	enemyarrayspottedtrigs = getentarray("enemyspotterarray","targetname");
	for(i=0;i<enemyarrayspottedtrigs.size;i++)
	{
		enemyarrayspottedtrigs[i] thread trigger_enemy_spotterarray();
	}
}
...
watersplashes()
{
	...
	if(!isdefined(self.script_team))
		sparkcount = 3;  //player tank doesn't require soo many splashes.
	while(self.health > 0)
	{
		...
		if(count > sparkcount)
		{
			for(i=0;i<self.samplepoints.size;i++)
			{
				...
				z = 14;  //level of water in tankdrivecountry;
...
tank_getmovingmessage()
{
	timertime = (level.getmovingmessagetime * 1000);
	timer = gettime()+ 1000;
	wait level.getmovingmessagetime;
	while(1)
	{
		if(self.speed == 0 && gettime() > timer)
		{
			self playsound ("tankdrive_hurryup");
