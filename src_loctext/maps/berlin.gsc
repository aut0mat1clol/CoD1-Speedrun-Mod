...
objectives_wait2()
{
	...
	cinematic("cod_end.roq");
		// [SR] freeze AFTER the cinematic() call: the call starts an
		// engine fade, the RoQ itself becomes visible ~0.6s later. 0.6s was
		// measured in the field (freeze lands exactly on the first video
		// frame). Hardcoded on purpose: fixed split point for all runners.
		wait (0.6);
		setcvar("rt_end_frozen", "1");	// [SR] freeze run timer when the end video starts
	}
}
