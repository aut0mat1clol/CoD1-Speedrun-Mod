...
main()
{
	...
	level.flags["HasExplosives"]	= true;
	level.flags["bunker_exit_open"]	= false;
	level.flags["barn_open"]	= false;
	level.flags["trench1_done"]	= false;
	level.flags["trench2_done"]	= false;
	level.flags["gave_explosives"]	= false;
	level.flaks_down = 0;
	...
	// one bombs thread only — wait, then we notify after dummy bombs are cleared
	...
	thread give_explosives_at_start();
	thread open_bunker_entrance();
	thread barn_after_three_flaks();
}

give_explosives_at_start()
{
	wait .1;
	if(isdefined(level.dummybomb))
	{
		for (i=0;i<level.dummybomb.size;i++)
		{
			if(isdefined(level.dummybomb[i].trigger))
				level.dummybomb[i].trigger delete();
			if(isdefined(level.dummybomb[i]))
				level.dummybomb[i] delete();
		}
	}
	level.flags["HasExplosives"] = true;
	level.flags["gave_explosives"] = true;
	level notify ("Has_Explosives");
}

open_bunker_entrance()
{
	wait .05;
...
open_bunker_exit()
{
	if(level.flags["bunker_exit_open"] == true)
		return;
	level.flags["bunker_exit_open"] = true;
	
	bunkerdoor2 = getent ("bunkerdoor2","targetname");
	if(isdefined(bunkerdoor2))
	{
		bunkerdoor2 connectpaths();
		bunkerdoor2 playsound ("wood_door_kick");
		bunkerdoor2 rotateyaw(-90, 0.4, 0.1, 0.1);
	}
}

enable_barn_area()
{
	if(level.flags["barn_open"] == true)
		return;
	level.flags["barn_open"] = true;
	
	trig = getent ("barnspawners_trigger1","targetname");
	if(isdefined(trig))
		trig thread maps\_utility::triggerOn();
	trig = getent ("barnspawners_trigger2","targetname");
	if(isdefined(trig))
		trig thread maps\_utility::triggerOn();
	trig = getent ("friendchain_crossfield","targetname");
	if(isdefined(trig))
		trig thread maps\_utility::triggerOn();
	trig = getent ("friendchainatbarn","targetname");
	if(isdefined(trig))
		trig thread maps\_utility::triggerOn();
...
count_flak_down(note)
{
	while(1)
	{
		level waittill (note);
		level.flaks_down++;
		if(level.flaks_down >= 3)
			return;
	}
}

barn_after_three_flaks()
{
	thread count_flak_down("Flak1_Blow");
	thread count_flak_down("Flak2_Blow");
	thread count_flak_down("bomb_trigger planted");
	
	while(level.flaks_down < 3)
		wait .2;
...
objective_wait()
{
	...
	for (i=0;i<reinforcements.size;i++)
	{
		...
		spawner.health = (600);
		if ( (isdefined (spawner.script_noteworthy)) && (spawner.script_noteworthy == "lieutenant") )
		{
	...
	// do not block bomb/barn logic on Moody walking to the door
	thread Moody_Kick_Sequence();
...
Moody_GetInjuredMan()
{
	...
	if (level.flags["moodydied"] == false)
	{
		...
		if (level.flags["gave_explosives"] == true)
		{
	...
	{	
		...
		if (level.flags["gave_explosives"] == false)
...
got_explosives()
{
	...
	if(isdefined(obj_flak1_explosives))
...
open_bunkerdoor()
{
	// entrance already opened at start — do not rotate again
	wait (1);
...
Trench_Explode()
{
	if(level.flags["trench1_done"] == true)
		return;
	level.flags["trench1_done"] = true;
	...
	{
		while (distance(level.player.origin, explosion.origin) < 30)
			wait 0.1;
	...
	if(isdefined(clipplayer))
		clipplayer delete();
}

Trench2_Explode()
{
	if(level.flags["trench2_done"] == true)
		return;
	level.flags["trench2_done"] = true;
	...
	if(isdefined(explosion2))
	{
		while (distance(level.player.origin, explosion2.origin) < 30)
			wait 0.1;
...
Moody_DeathwithWounded()
{
	...
	while (dmgtaken < level.moody_woundedsequence_health)
	{
		...
		{
			if ( (isdefined (level.moody.damagelocation)) && (level.moody.damagelocation == "none") )
				continue;
