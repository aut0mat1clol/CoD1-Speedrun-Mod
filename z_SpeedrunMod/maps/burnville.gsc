...
//	level.player.threatbias = -20000;
	...
	maps\_utility::array_levelthread (getentarray ("blocker","targetname"), ::blocker);	

	//thread temp();
//	thread temp_bullet();	
	level.ambient_track ["exterior"] = "ambient_burnville";
	level.ambient_track ["interior"] = "ambient_burnville_int";
...
//	getent ("halftrack_kill","targetname") thread halftrack_kill(getent ("halftrack_gunner", "targetname"));
	...
	level thread killBlocker (getent ("kill blocker","targetname"));
	maps\_utility::array_levelthread(getentarray ("trigger flak","targetname"), ::trigger_flak);
	maps\_utility::array_levelthread(getentarray ("trigger_multiple","classname"), ::preSpawnAlive);
...
{
	trigger waittill ("trigger");
	getent ("blocker","targetname") connectpaths();
	trigger delete();
}
...
flak_dialogue_think ( trigger, obj, msg, optionalEnemyTrigger )
{
	...
	if (isdefined (optionalEnemyTrigger))
	{
		...
		waitMsg = waittill_either (level, obj + "bomb placed", "killed the ai");
		if (waitMsg == "killed the ai")
		{
...
{
	if ((spawner.classname[0] == "s") &&
		(spawner.classname[1] == "c") &&
		(spawner.classname[2] == "r"))
		return;
...
preSpawnAlive ( trigger )
{
	...
	trigger waittill ("trigger");
	trigger delete();
	maps\_utility::array_levelthread(spawners, ::preSpawnThink, spawnTrigger);
}
