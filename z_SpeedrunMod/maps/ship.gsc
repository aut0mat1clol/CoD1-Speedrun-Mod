main()
{
	// 1 = skip dinghy approach cinematic, start already at the ship
	// 2 = also skip papers check, start following Price on deck
	level.skip_opening = 1;
...
//	thread maps\_adjustfog::adjustExpFog(); //temp
	...
	// skipped: ship_boat_ride (long approach loop)
...
opening_sequence()
{
	...
	// +Z so the player is not buried in the dinghy hull after skip
	level.player linkTo(level.boat, "tag_player", (40,0,28), (0,0,0));
	...
	if(isdefined(level.skip_opening) && level.skip_opening >= 1)
	{
		if(isdefined(level.deadgerman))
			level.deadgerman delete();

		// opening_loop is the docked idle, authored on boatnode — skip the approach anim
		thread maps\_anim::anim_loop(boatrig, "opening_loop", undefined, undefined, boatnode);

		waters[0] = level.waters;
		level.boat thread maps\_anim::anim_loop(waters, "opening_loop", "tag_boat", "endloop_waters");
		...
		// drop Price on deck so he does not path out of the dinghy
		boardnode = getnode("boardnode", "targetname");
		if(isdefined(boardnode))
			level.price teleport(boardnode.origin, boardnode.angles);
...
boarding_sequence()
{
	...
	if(isdefined(level.skip_opening) && level.skip_opening >= 2)
	{
...
hangar_sequence()
{
	...
	level.pistolguy thread kill();
	level.pistolguy delete();
	level.mp40guy delete();
...
goto_boat_objective()
{
	...
	missionSuccess("stalingrad", false);
